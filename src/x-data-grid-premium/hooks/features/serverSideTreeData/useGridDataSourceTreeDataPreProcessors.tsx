...
export const useGridDataSourceTreeDataPreProcessors = (
  ...
  const getGroupingColDef = React.useCallback(() => {
    ...
    const commonProperties: Omit<GridColDef, "field" | "editable"> = {
      ...
      headerName: privateApiRef.current.getLocaleText(
    ...
    (columnsState) => {
      ...
      if (shouldHaveGroupingColumn) {
        ...
        if (prevGroupingColumn == null) {
          const index =
            columnsState.orderedFields[0] === GRID_CHECKBOX_SELECTION_FIELD
              ? 1
              : 0;
  ...
  const createRowTreeForTreeData = React.useCallback<
    GridStrategyProcessor<"rowTreeCreation">
  >(
    (params) => {
      const getGroupKey = props.unstable_dataSource?.getGroupKey;
      if (!getGroupKey) {
        throw new Error(
          "MUI X: No `getGroupKey` method provided with the dataSource."
        );
      }
      ...
      if (!getChildrenCount) {
        throw new Error(
          "MUI X: No `getChildrenCount` method provided with the dataSource."
        );
      }
      ...
      const onDuplicatePath: GridTreePathDuplicateHandler = (
        firstId,
        secondId,
        path
      ) => {
  ...
  const filterRows = React.useCallback<
    GridStrategyProcessor<"filtering">
  >(() => {
