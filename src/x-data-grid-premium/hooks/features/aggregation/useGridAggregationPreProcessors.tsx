...
export const useGridAggregationPreProcessors = (
  ...
  const updateAggregatedColumns = React.useCallback<
  ...
  >(
    (columnsState) => {
      ...
      columnsState.orderedFields.forEach((field) => {
        ...
        const haveAggregationColumnValue =
          !!rulesOnLastColumnHydration.current[field];
  ...
  >(
    (value) => {
      ...
      if (
        Object.keys(apiRef.current.caches.aggregation.rulesOnLastRowHydration)
          .length === 0 &&
        !hasAggregationRule
      ) {
        return value;
      ...
      apiRef.current.caches.aggregation.rulesOnLastRowHydration =
        aggregationRules;
    ...
    (
      ...
      if (aggregationModel != null) {
        apiRef.current.setState(
          mergeStateWithAggregationModel(aggregationModel)
        );
      }
      return params;
    },
