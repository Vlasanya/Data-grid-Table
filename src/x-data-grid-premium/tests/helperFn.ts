...
export function gridOffsetTop() {
  const transform = getComputedStyle(
    grid("virtualScrollerRenderZone")!
  ).transform;
  return parseInt(
...
export function getActiveColumnHeader() {
  ...
  if (
    ...
    activeElement =
      document.activeElement &&
      document.activeElement.closest('[role="columnheader"]');
  }
...
export function getColumnHeaderCell(
  colIndex: number,
  rowIndex?: number
): HTMLElement {
...
export function getRowsFieldContent(field: string) {
  return Array.from(
    ...
    (node) =>
      node.querySelector(`[role="gridcell"][data-field="${field}"]`)
        ?.textContent
  );
}
