...
export const getGroupRowIdFromPath = (
  path: RowTreeBuilderGroupingCriterion[]
) => {
  const pathStr = path
    .map(
      (groupingCriteria) => `${groupingCriteria.field}/${groupingCriteria.key}`
    )
    .join("-");
...
export const insertNodeInTree = (
  ...
  if (node.type === "group" || node.type === "leaf") {
    ...
    const groupingFieldName =
      (node as GridGroupNode).groupingField ?? "__no_field__";
    ...
    if (
      previousTree !== null &&
      previousTree[parentNode.id] === tree[parentNode.id]
    ) {
...
export const removeNodeFromTree = ({
  ...
  // Unregister the node from its parents `children` and `childrenFromPath` properties.
    ...
    const childrenFromPath: GridChildrenFromPathLookup =
