...
export const sortRowTree = (params: SortRowTreeParams) => {
  ...
  const sortGroup = (node: GridGroupNode) => {
    const shouldSortGroup =
      !!sortRowList && (!disableChildrenSorting || node.depth === -1);
    ...
    if (shouldSortGroup) {
      ...
      sortedRowIds = sortRowList(
        node.children.map((childId) => rowTree[childId])
      );
  ...
  const rootList = List.from<GridRowId>(
    sortedGroupedByParentRows.get(GRID_ROOT_GROUP_ID)!
  );
