...
export interface GridHeaderFilterCellProps
...
const useUtilityClasses = (ownerState: OwnerState) => {
  const { colDef, classes, showRightBorder, showLeftBorder, pinnedPosition } =
    ownerState;
...
>((props, ref) => {
  ...
  const editingField = useGridSelector(
    apiRef,
    gridHeaderFilteringEditFieldSelector
  );
  const isEditing = editingField === colDef.field;

  const menuOpenField = useGridSelector(
    apiRef,
    gridHeaderFilteringMenuSelector
  );
  ...
  const filterOperators = React.useMemo(() => {
    ...
    return colDef.filterOperators.filter(
  ...
  const isFilterReadOnly = React.useMemo(() => {
    ...
    const filterModelItem = filterModel.items.find(
      (it) => it.field === colDef.field
    );
    return filterModelItem
      ? !filterableColumnsLookup[filterModelItem.field]
      : false;
  }, [colDef.field, filterModel, filterableColumnsLookup]);

  const currentOperator = React.useMemo(
    () =>
      filterOperators.find((operator) => operator.value === item.operator) ??
      filterOperators![0],
  ...
  const InputComponent =
    colDef.filterable || isFilterReadOnly
      ? (currentOperator.InputComponent ??
        defaultInputComponents[colDef.type as GridColType])
      : null;
    ...
    (event: React.KeyboardEvent) => {
      ...
      switch (event.key) {
        ...
        case "Tab": {
          if (isEditing) {
            const fieldToFocus =
              columnFields[colIndex + (event.shiftKey ? -1 : 1)] ?? null;
