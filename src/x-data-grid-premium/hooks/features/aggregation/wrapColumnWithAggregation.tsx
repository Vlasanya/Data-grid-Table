...
import { GridColDef, GridFilterOperator, GridRowId } from "@mui/x-data-grid";
...
type WrappableColumnProperty =
  (typeof AGGREGATION_WRAPPABLE_PROPERTIES)[number];
...
const getAggregationValueWrappedValueGetter: ColumnPropertyWrapper<
  "valueGetter"
> = ({ value: valueGetter, getCellAggregationResult }) => {
  const wrappedValueGetter: GridBaseColDef["valueGetter"] = (
    value,
    row,
    column,
    apiRef
    ...
    const cellAggregationResult = rowId
      ? getCellAggregationResult(rowId, column.field)
      : null;
...
const getAggregationValueWrappedValueFormatter: ColumnPropertyWrapper<
  "valueFormatter"
> = ({ value: valueFormatter, aggregationRule, getCellAggregationResult }) => {
  ...
  const wrappedValueFormatter: GridBaseColDef["valueFormatter"] = (
    value,
    row,
    column,
    apiRef
    ...
    const rowId = apiRef.current.getRowId(row);
    if (rowId != null) {
      const cellAggregationResult = getCellAggregationResult(
        rowId,
        column.field
      );
      if (cellAggregationResult != null) {
...
const getAggregationValueWrappedRenderCell: ColumnPropertyWrapper<
  "renderCell"
> = ({ value: renderCell, aggregationRule, getCellAggregationResult }) => {
...
const getWrappedFilterOperators: ColumnPropertyWrapper<"filterOperators"> = ({
  ...
  filterOperators!.map((operator) => {
    ...
    const getApplyFilterFn: GridFilterOperator<
      any,
      any,
      any
    >["getApplyFilterFn"] = (filterItem, colDef) => {
...
export const wrapColumnWithAggregationValue = ({
  ...
  const getCellAggregationResult = (
    ...
    const groupId =
      cellAggregationPosition === "inline" ? id : (rowNode.parent ?? "");

    const aggregationResult =
      gridAggregationLookupSelector(apiRef)?.[groupId]?.[field];
    if (
      !aggregationResult ||
      aggregationResult.position !== cellAggregationPosition
    ) {
      return null;
    }
  ...
  wrapColumnProperty(
    "valueFormatter",
    getAggregationValueWrappedValueFormatter
  );
...
export const unwrapColumnFromAggregation = ({
  ...
  if (
    !(column as GridColDefWithAggregationWrappers).aggregationWrappedProperties
  ) {
  ...
  aggregationWrappedProperties.forEach(
    ({ name, originalValue, wrappedValue }) => {
