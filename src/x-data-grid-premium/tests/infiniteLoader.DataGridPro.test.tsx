...
describe('<DataGridPro /> - Infnite loader', () => {
  it('should call `onRowsScrollEnd` when viewport scroll reaches the bottom', async function test() {
    ...
    const { container, rerender } = render(<TestCase rows={baseRows} />);
    ...
    await waitFor(() => {
      expect(handleRowsScrollEnd.callCount).toEqual(1);
    });
    ...
    await waitFor(() => {
      expect(handleRowsScrollEnd.callCount).toEqual(2);
    });
  });
  ...
  it('should call `onRowsScrollEnd` when there is not enough rows to cover the viewport height', async function test() {
    ...
    await waitFor(() => {
      expect(getRow.callCount).toEqual(1);
    });
    await waitFor(() => {
    ...
    await waitFor(() => {
      expect(getRow.callCount).toEqual(2);
    });
    await waitFor(() => {
    ...
    await waitFor(() => {
      expect(getRow.callCount).toEqual(3);
    });
    await waitFor(() => {
      expect(getColumnValues(0)).toEqual(['0', '1', '2', '3']);
    });

    await waitFor(() => {
      expect(getRow.callCount).toEqual(4);
    });
    await waitFor(() => {
    ...
    await waitFor(() => {
      expect(getRow.callCount).toEqual(5);
    });
    await waitFor(() => {
  ...
  it('should not observe intersections with the rows pinned to the bottom', async function test() {
    ...
    expect(observe.callCount).toEqual(0);
    // arbitrary number to make sure that the bottom of the grid window is reached.
    virtualScroller.scrollTop = 12345;
    virtualScroller.dispatchEvent(new Event('scroll'));
    ...
    expect(observe.callCount).toEqual(1);
  });
});