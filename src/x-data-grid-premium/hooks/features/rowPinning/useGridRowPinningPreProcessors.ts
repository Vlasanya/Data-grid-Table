...
export function addPinnedRow({
  ...
  if (
    groupingParams.additionalRowGroups?.pinnedRows?.[position]?.includes(
      newPinnedRow
    )
  ) {
    return {
      ...
      if (pinnedRowsCache.bottomIds?.length || pinnedRowsCache.topIds?.length) {
        const shouldKeepRow = (rowId: GridRowId) => {
          if (
            ...
            newGroupingParams.tree[rowId].type === "pinnedRow"
          ) {
            return false;
          }
          return true;
        ...
        const rootGroupNode = newGroupingParams.tree[
          GRID_ROOT_GROUP_ID
        ] as GridGroupNode;
        ...
        newGroupingParams.dataRowIds =
          newGroupingParams.dataRowIds.filter(shouldKeepRow);
      }
