...
const getFormattedValueOptions = (
  ...
  if (colDef.valueFormatter) {
    valueOptionsFormatted = valueOptionsFormatted.map((option) => {
      ...
      return String(
        colDef.valueFormatter!(option as never, row, colDef, { current: api })
      );
    });
  }
  ...
  columns.forEach((column, colIndex) => {
    ...
    switch (cellParams.colDef.type) {
      case "singleSelect": {
        ...
        const formattedValue = apiRef.current.getCellParams(
          id,
          castColumn.field
        ).formattedValue;
      ...
      case "dateTime": {
        ...
        const value = apiRef.current.getCellParams<any, Date>(
          id,
          column.field
        ).value;
      ...
      case "actions":
        break;
      default:
        cellValue = apiRef.current.getCellParams(id, column.field)
...
export const serializeColumn = (
  ...
  return {
    ...
    style: {
      ...(type && defaultColumnsStyles?.[type]),
      ...columnsStyles?.[field],
    },
  };
};
...
const addColumnGroupingHeaders = (
  ...
  const maxDepth = Math.max(
    ...columns.map(({ key }) => columnGroupPaths[key]?.length ?? 0)
  );
  if (maxDepth === 0) {
    return;
  }
  ...
  for (let rowIndex = 0; rowIndex < maxDepth; rowIndex += 1) {
    ...
    const newRow = worksheet.addRow(
      row.map((group) =>
        group.groupId === null ? null : (group?.headerName ?? group.groupId)
      )
    );
    ...
    while (rightIndex < columns.length) {
      ...
      const areInSameGroup =
        ...
        leftParents.every(
          (leftParent, index) => rightParents[index] === leftParent
        );
      if (areInSameGroup) {
        rightIndex += 1;
      } else {
...
type ValueOptionsData = Record<
  string,
  { values: (string | number)[]; address: string }
>;
...
export async function getDataForValueOptionsSheet(
  ...
  const candidateColumns = columns.filter(
    (column) =>
      isSingleSelectColDef(column) && Array.isArray(column.valueOptions)
  );
  ...
  return candidateColumns.reduce<
    Record<string, { values: (string | number)[]; address: string }>
  >((acc, column) => {
...
function addSerializedRowToWorksheet(
  ...
  mergedCells.forEach((mergedCell) => {
    worksheet.mergeCells(
      lastRowIndex,
      mergedCell.leftIndex,
      lastRowIndex,
      mergedCell.rightIndex
    );
  });
}
...
interface BuildExcelOptions
  extends Pick<
      GridExcelExportOptions,
      "exceljsPreProcess" | "exceljsPostProcess"
    >,
    Pick<
...
export async function buildExcel(
  ...
  if (includeColumnGroupsHeaders) {
    const columnGroupPaths = columns.reduce<Record<string, string[]>>(
      (acc, column) => {
