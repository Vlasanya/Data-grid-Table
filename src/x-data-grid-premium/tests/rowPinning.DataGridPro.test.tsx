...
describe("<DataGridPremium /> - Row pinning", () => {
  ...
  it("should treat row as pinned even if row with the same id is present in `rows` prop", () => {
    ...
    function TestCase({ pinRows = true }) {
      ...
      return (
        <div style={{ width: 302, height: 300 }}>
          <DataGridPremium {...data} autoHeight pinnedRows={pinnedRows} />
        </div>
      );
    }
    ...
    expect(isRowPinned(getRowById(0), "top")).toEqual(true);
    expect(isRowPinned(getRowById(1), "bottom")).toEqual(true);
    expect(getColumnValues(0)).toEqual(["0", "2", "3", "4", "1"]);
    expect(screen.getByText(`Total Rows: ${rowCount - 2}`)).not.toEqual(null);
    rerender(<TestCase pinRows={false} />);

    expect(isRowPinned(getRowById(0), "top")).toEqual(false);
    expect(isRowPinned(getRowById(1), "bottom")).toEqual(
      false
      // "#1 not pinned"
    );
    expect(getColumnValues(0)).toEqual(["0", "1", "2", "3", "4"]);
    expect(screen.getByText(`Total Rows: ${rowCount}`)).not.toEqual(null);

    rerender(<TestCase pinRows={true} />);
  ...
  it("should update pinned rows when `pinnedRows` prop change", () => {
    ...
    rerender(<TestCase pinnedRows={true} rows={true} />);
  ...
  it("should update pinned rows when calling `apiRef.current.setPinnedRows` method", async () => {
    ...
    expect(isRowPinned(getRowById(11), "top")).toEqual(true);
    expect(isRowPinned(getRowById(3), "bottom")).toEqual(
      true
      // "#3 pinned bottom"
    );
    ...
    expect(isRowPinned(getRowById(11), "top")).toEqual(
      false
      // "#11 pinned top"
    );
    expect(isRowPinned(getRowById(3), "bottom")).toEqual(
    ...
    expect(isRowPinned(getRowById(8), "top")).toEqual(true);
    expect(isRowPinned(getRowById(5), "bottom")).toEqual(
      true
      // "#5 pinned bottom"
    );
  });
  ...
  it("should work with `getRowId`", () => {
    function TestCase() {
      ...
      const getRowId = React.useCallback<
        NonNullable<DataGridPremiumProps["getRowId"]>
      >((row) => row.productId, []);
  ...
  it("should not be impacted by sorting", () => {
    ...
    expect(getColumnValues(0)).toEqual(["0", "4", "3", "2", "1"]);
  });

  it("should not be impacted by filtering", () => {
    const { rerender } = render(
      <BaselineTestCase rowCount={20} colCount={5} />
    );
    ...
    rerender(
      <BaselineTestCase
        ...
        filterModel={{
          items: [
            { field: "currencyPair", operator: "equals", value: "GBPEUR" },
          ],
        }}
      />
    );
    ...
    //   filterModel: {
    //     items: [
    //       { field: "currencyPair", operator: "equals", value: "whatever" },
    //     ],
    //   },
    // });
    rerender(
  ...
  describe("keyboard navigation", () => {
    ...
    it("should work with top pinned rows", () => {
      ...
      expect(getActiveColumnHeader()).toEqual("1");
    });
    ...
    it("should work with bottom pinned rows", () => {
      ...
      expect(isRowPinned(getRowById(0), "bottom")).toEqual(
        true
        // "#0 pinned top"
      );
      expect(isRowPinned(getRowById(1), "bottom")).toEqual(
      ...
      fireEvent.keyDown(getCell(0, 0), { key: "ArrowDown" });
      expect(getActiveCellRowId()).toEqual("3");

      fireEvent.keyDown(getCell(1, 0), { key: "ArrowDown" });
      expect(getActiveCellRowId()).toEqual("4");
    ...
    it("should work with pinned columns", function test() {
      ...
      expect(isRowPinned(getRowById(1), "top")).toEqual("#1 pinned top");
      expect(isRowPinned(getRowById(0), "bottom")).toEqual(
        // true,
        "#0 pinned bottom"
      );
  ...
  it("should work with variable row height", function test() {
    ...
    expect(getRowById(0)?.clientHeight).toEqual(100);
    expect(getRowById(1)?.clientHeight).toEqual(20);
  });
  ...
  it("should always update on `rowHeight` change", async function test() {
    ...
    expect(getRowById(0)!.offsetHeight).toEqual(defaultRowHeight);
    expect(grid("pinnedRows--top")!.offsetHeight).toEqual(defaultRowHeight);
    expect(getRowById(1)!.clientHeight).toEqual(defaultRowHeight);
    expect(grid("pinnedRows--bottom")!.offsetHeight).toEqual(defaultRowHeight);

    // setProps({ rowHeight: 36 });
    rerender(<TestCase rowHeight={36} />);

    expect(getRowById(0)?.clientHeight).toEqual(36);
    expect(grid("pinnedRows--top")!.offsetHeight).toEqual(36);
    expect(getRowById(1)?.clientHeight).toEqual(36);
    expect(grid("pinnedRows--bottom")!.offsetHeight).toEqual(36);
  });
  ...
  it("should work with `autoHeight`", function test() {
    ...
    expect(grid("main")!.clientHeight).toEqual(
  ...
  it("should work with `autoPageSize`", function test() {
    ...
    expect(getRows().length).toEqual(4);
  });
  ...
  it("should not allow to reorder pinned rows", () => {
    ...
    const cell = getCell(0, 0);
    expect(cell.querySelector(`.${gridClasses.rowReorderCell}`)).toEqual(null);
  });
  ...
  it("should not count pinned rows as part of the page", () => {
    ...
    expect(getRows().length).toEqual(pageSize + 2); // + 2 pinned rows
  });
  ...
  it("should not render selection checkbox for pinned rows", () => {
    ...
    expect(getRowById(0)!.querySelector('input[type="checkbox"]')).toEqual(
      null
    );
    expect(getRowById(1)!.querySelector('input[type="checkbox"]')).toEqual(
      null
    );
  });
  ...
  it("should export pinned rows to CSV", () => {
    ...
    expect(csvRows[0]).toEqual("0");
    expect(csvRows[csvRows.length - 1]).toEqual("1");
  });
  ...
  it("should work with `getCellClassName` when `rows=[]`", () => {
    ...
    expect(getRowById(0)!).toHaveClass(className);
    expect(getRowById(1)!).toHaveClass(className);
  });
  ...
  it("should support cell editing", async function test() {
    ...
    await waitFor(() => {
      expect(cell.textContent).toEqual("Marcus");
    });
  ...
  it("should support `updateRows`", async () => {
    ...
    expect(getCell(0, 1).textContent).toEqual("Joe");
    expect(getCell(4, 1).textContent).toEqual("Cory");
