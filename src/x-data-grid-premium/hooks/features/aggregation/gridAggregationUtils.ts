...
import { addPinnedRow } from "../rowPinning/useGridRowPinningPreProcessors";
import {
  GridColumnRawLookup,
...
export const GRID_AGGREGATION_ROOT_FOOTER_ROW_ID =
  "auto-generated-group-footer-root";

export const getAggregationFooterRowIdFromGroupId = (
  groupId: GridRowId | null
) => {
  if (groupId == null) {
...
export const canColumnHaveAggregationFunction = ({
  ...
  if (colDef.availableAggregationFunctions != null) {
    return colDef.availableAggregationFunctions.includes(
      aggregationFunctionName
    );
  }
...
export const addFooterRows = ({
  ...
  const updateChildGroupFooter = (groupNode: GridGroupNode) => {
    const shouldHaveFooter =
      hasAggregationRule && getAggregationPosition(groupNode) === "footer";
    ...
    if (shouldHaveFooter) {
      ...
      if (groupNode.footerId !== footerId) {
        ...
        insertNodeInTree(
          footerNode,
          newGroupingParams.tree,
          newGroupingParams.treeDepths,
          null
        );
      }
  ...
  updateGroupFooter(
    newGroupingParams.tree[GRID_ROOT_GROUP_ID] as GridGroupNode
  );
...
export const areAggregationRulesEqual = (
  ...
  return newFields.every((field) => {
    ...
    if (
      previousRule?.aggregationFunctionName !== newRule?.aggregationFunctionName
    ) {
      return false;
    }
