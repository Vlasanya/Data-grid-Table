  ...
  >(
    (columnsState) => {
      ...
      apiRef.current.state = {
        ...savedState,
        columns: columnsState as unknown as any,
      };
      ...
      const { orderedFieldsBeforePinningColumns } =
        apiRef.current.caches.columnPinning;

      if (orderedFieldsBeforePinningColumns) {
        newOrderedFields = new Array(columnsState.orderedFields.length).fill(
          null
        );
      ...
      const centerColumns = newOrderedFields.filter((field) => {
        return (
          !leftPinnedColumns.includes(field) &&
          !rightPinnedColumns.includes(field)
        );
      });
