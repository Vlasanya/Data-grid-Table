...
export const useGridRowGrouping = (
  ...
  const setRowGroupingModel = React.useCallback<
    GridRowGroupingApi["setRowGroupingModel"]
  >(
    (model) => {
  ...
  const addRowGroupingCriteria = React.useCallback<
    GridRowGroupingApi["addRowGroupingCriteria"]
  >(
    (field, groupingIndex) => {
      const currentModel = gridRowGroupingModelSelector(apiRef);
  ...
  >(
    (field) => {
      ...
      apiRef.current.setRowGroupingModel(
        currentModel.filter((el) => el !== field)
      );
    },
    [apiRef]
  );
  ...
  >(
    (field, targetIndex) => {
      ...
      updatedModel.splice(
        targetIndex,
        0,
        updatedModel.splice(currentTargetIndex, 1)[0]
      );
  ...
  >(
    (
      params,
      context: GridRestoreStatePreProcessingContext<GridInitialStatePremium>
    ) => {
      if (props.disableRowGrouping) {
        return params;
      }
      ...
      if (rowGroupingModel != null) {
        apiRef.current.setState(
          mergeStateWithRowGroupingModel(rowGroupingModel)
        );
      }
      return params;
    },
  ...
  const handleCellKeyDown = React.useCallback<GridEventListener<"cellKeyDown">>(
    (params, event) => {
      ...
      if (
        isGroupingColumn(cellParams.field) &&
        event.key === " " &&
        !event.shiftKey
      ) {
  ...
  >(() => {
    const sanitizedRowGroupingModel =
      gridRowGroupingSanitizedModelSelector(apiRef);
    ...
    if (!areGroupingRulesEqual(rulesOnLastRowTreeCreation, groupingRules)) {
      ...
      if (
        apiRef.current.getActiveStrategy("rowTree") === ROW_GROUPING_STRATEGY
      ) {
