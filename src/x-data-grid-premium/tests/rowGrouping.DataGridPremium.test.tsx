...
// export declare const getGroupRowIdFromPath: (path: any) => string;
...
describe("<DataGridPremium /> - Row grouping", () => {
  ...
  describe("Setting grouping criteria", () => {
    ...
    describe("initialState: rowGrouping.model", () => {
      ...
      //   ]);
      //   rerender(
      //     <Test initialState={{ rowGrouping: { model: ["category2"] } }} />
      //   );
      //   expect(getColumnValues(0)).toEqual([
      //     "Cat A (3)",
      //     "",
    ...
    describe("prop: rowGroupingModel", () => {
      it("should not call onRowGroupingModelChange on initialisation or on rowGroupingModel prop change", () => {
        const onRowGroupingModelChange = jest.fn();
  ...
  describe("prop: rowGroupingColumnMode", () => {
    ...
    //   rerender(<Test rowGroupingColumnMode="single" />);
    //   expect(getColumnHeadersTextContent()).toEqual([
    //     "Group",
    //     "id",
    //     "category1",
    ...
    //   rerender(<Test rowGroupingColumnMode="multiple" />);
    //   expect(getColumnHeadersTextContent()).toEqual([
    //     "category1",
    //     "category2",
    //     "id",
  ...
  describe("prop: disableRowGrouping", () => {
    ...
    //   expect(apiRef.current.state.rows.groupingName).toEqual("none");
    ...
    //   });
    //   expect(category1Menuitem).toEqual(null);
    ...
    //   });
    //   expect(screen.queryByRole("menu")).not.toEqual(null);
    //   const category2Menuitem = screen.queryByRole("menuitem", {
    //     name: "Group by category2",
    //   });
    //   expect(category2Menuitem).toEqual(null);
    // });
  });
  ...
  describe("prop: defaultGroupingExpansionDepth", () => {
    ...
    it("should not expand any row if defaultGroupingExpansionDepth = 0", () => {
      ...
      expect(getColumnValues(0)).toEqual(["Cat A (3)", "Cat B (2)"]);
    });
    ...
    it("should not re-apply default expansion on rerender after expansion manually toggled", () => {
      ...
      rerender(
        <Test
          sortModel={[
            { field: "__row_group_by_columns_group__", sort: "desc" },
          ]}
        />
      );
      expect(getColumnValues(0)).toEqual([
  ...
  describe("prop: isGroupExpandedByDefault", () => {
    ...
    //     />
    //   );
    //   expect(isGroupExpandedByDefault.mock.calls.length).toEqual(12);

    //   const { childrenExpanded, ...node } = apiRef.current.state.rows.tree
    //     .A as GridGroupNode;
    //   const callForNodeA = isGroupExpandedByDefault.mock.calls.find(
    //     (call: any) =>
    //       call[0].groupingKey === "Cat A" &&
    //       call[0].groupingField === "category1"
    //   );
    //   expect(callForNodeA).toBeDefined();
  ...
  describe('prop: groupingColDef when groupingColumnMode = "single"', () => {
    ...
    it("should not allow to override the field", () => {
      ...
      expect(apiRef.current.getAllColumns()[0].field).toEqual(
    ...
    it("should react to groupingColDef update", () => {
      ...
      rerender(
        <Test
          initialState={{ rowGrouping: { model: ["category1", "category2"] } }}
          groupingColDef={{ headerName: "Custom group" }}
        />
      );
      expect(getColumnHeadersTextContent()).toEqual([
        "Custom group",
    ...
    describe("prop: groupColDef.leafField", () => {
      ...
      it("should render the leafField `renderCell` on leaves  if `renderCell` is defined on the leafColDef", () => {
        const renderIdCell = jest.fn(() => "Custom leaf");
      ...
      it("should correctly pass `hasFocus` to `renderCell` defined on the leafColDef", () => {
        const renderIdCell = jest.fn(
          (params: any) => `Focused: ${params.hasFocus}`
        );
        ...
        const lastCallParams =
          renderIdCell.mock.calls[renderIdCell.mock.calls.length - 1][0];
        expect(lastCallParams.field).toEqual("id");
        expect(getCell(1, 0)).toHaveTextContent("Focused: true");
      });
    });
  ...
  describe('prop: groupingColDef when groupingColumnMode = "multiple"', () => {
    ...
    it("should keep the grouping column width between generations", () => {
      ...
      expect(getColumnHeaderCell(0)).toHaveStyle({ width: "200px" });
      expect(getColumnHeaderCell(1)).toHaveStyle({ width: "300px" });
      act(() =>
        apiRef.current.updateColumns([
          {
            field: getRowGroupingFieldFromGroupingCriteria("category1"),
            width: 100,
          },
        ])
      );
  ...
  describe("colDef: groupingValueGetter & valueGetter", () => {
    ...
    it("should use groupingValueGetter to group rows when defined", () => {
      ...
      expect(getColumnValues(1)).toEqual(["", "0", "1", "2", "", "3", "4"]);
    });
    ...
    it("should react to groupingValueGetter update", () => {
      ...
      expect(getColumnValues(1)).toEqual(["", "0", "2", "4", "", "1", "3"]);
      ...
      expect(getColumnValues(1)).toEqual(["", "0", "3", "", "1", "4", "", "2"]);
    });
  ...
  describe("column menu", () => {
    ...
    it('should add a "Group by {field}" menu item on ungrouped columns when coLDef.groupable is not defined', () => {
      ...
      const menuItem = screen.getByRole("menuitem", {
        name: "Group by category1",
      });
      fireEvent.click(menuItem);
      expect(apiRef.current.state.rowGrouping.model).toEqual(["category1"]);
    });
    ...
    it('should not add a "Group by {field}" menu item on ungrouped columns when coLDef.groupable = false', () => {
      ...
      expect(screen.queryByRole("menu")).not.toEqual(null);
      expect(
        screen.queryByRole("menuitem", { name: "Group by category1" })
      ).toEqual(null);
    });
    ...
    it('should add a "Stop grouping by {field}" menu item on grouped column', () => {
      ...
      fireEvent.click(menuItem);
      expect(apiRef.current.state.rowGrouping.model).toEqual([]);
    });
    ...
    it('should add a "Stop grouping by {field} menu item on each grouping column when prop.rowGroupingColumnMode = "multiple"', () => {
      ...
      expect(apiRef.current.state.rowGrouping.model).toEqual(["category2"]);
      ...
      act(() => {
        apiRef.current.showColumnMenu(
          "__row_group_by_columns_group_category2__"
        );
        jest.runAllTimers();
      });
      expect(screen.queryByRole("menu")).not.toEqual(null);
    ...
    it('should add a "Stop grouping {field}" menu item for each grouping criteria with colDef.groupable = false but it should be disabled', () => {
      ...
      expect(menuItemCategory1).toHaveClass("Mui-disabled");
      const menuItemCategory2 = screen.getByRole("menuitem", {
        name: "Stop grouping by category2",
      });
      expect(menuItemCategory2).toHaveClass("Mui-disabled");
    });
    ...
    it("should use the colDef.headerName property for grouping menu item label", () => {
      ...
      expect(screen.queryByRole("menu")).not.toEqual(null);
      expect(
        screen.queryByRole("menuitem", { name: "Group by Category 1" })
      ).not.toEqual(null);
    });
    ...
    it("should use the colDef.headerName property for ungrouping menu item label", () => {
      ...
      expect(screen.queryByRole("menu")).not.toEqual(null);
      expect(
        screen.queryByRole("menuitem", { name: "Stop grouping by Category 1" })
      ).not.toEqual(null);
    });
  });
  ...
  describe("filtering", () => {
    ...
    describe('prop: rowGroupingColumnMode = "single"', () => {
      ...
      it("should not filter the groups when filtering with an item that is not on the grouping column", () => {
        ...
        expect(getColumnValues(0)).toEqual(["Cat A (1)", "Cat 2 (1)", ""]);
      });
      ...
      it("should apply quick filter without throwing error", () => {
        ...
        expect(getColumnValues(1)).toEqual(["", "3", "4"]);
      });
      ...
      it("should let group appears when a leaf rows pass quick filter", () => {
        ...
        expect(getColumnValues(1)).toEqual(["", "0", "", "4"]);
      });
      ...
      it("should show all children when a group pass quick filter", () => {
        ...
        expect(getColumnValues(1)).toEqual(["", "0", "1", "2"]);
      });
      ...
      it("should manage link operator OR across group and leaf columns", () => {
        ...
        expect(getColumnValues(1)).toEqual(["", "0", "1", "2", "", "4"]);
      });
      ...
      it("should keep the correct count of the children and descendants in the filter state", () => {
        ...
        expect(
          filteredChildrenCountLookup["auto-generated-row-category1/Cat A"]
        ).toEqual(2);
        expect(
          filteredDescendantCountLookup["auto-generated-row-category1/Cat A"]
        ).toEqual(5);
    ...
    describe('prop: rowGroupingColumnMode = "multiple"', () => {
      it("should use the column grouping criteria for filtering if mainGroupingCriteria and leafField are not defined", () => {
        ...
        expect(getColumnValues(0)).toEqual(["Cat A (3)", "", "", ""]);
        expect(getColumnValues(1)).toEqual(["", "0", "1", "2"]);
      });
      ...
      it("should use the column grouping criteria for filtering if mainGroupingCriteria matches the column grouping criteria and leaf field is defined", () => {
        ...
        expect(getColumnValues(0)).toEqual(["Cat A (3)", "0", "1", "2"]);
      });
      ...
      it("should use the leaf field for filtering if mainGroupingCriteria is not defined and leaf field is defined", () => {
        ...
        expect(getColumnValues(0)).toEqual(["Cat B (2)", "3", "4"]);
      });
      ...
      it("should not filter the groups when filtering with an item that is not on the grouping column", () => {
        ...
        // "Cat A" & "Cat 2" groups are not tested against the "id" filter item
        expect(getColumnValues(0)).toEqual(["Cat A (1)", "", ""]);
        expect(getColumnValues(1)).toEqual(["", "Cat 2 (1)", ""]);
      });
    ...
    it("should not apply filters when the row is expanded", () => {
      ...
      const onFilteredRowsSet = jest.fn();
  ...
  describe("apiRef: getRowGroupChildren", () => {
    ...
    it("should return the rows in group of depth 0 of length 1 from tree of depth 1", () => {
      ...
      expect(apiRef.current.getRowGroupChildren({ groupId })).toEqual([
        0, 1, 2,
      ]);
      expect(
        apiRef.current.getRowGroupChildren({ groupId, applySorting: true })
      ).toEqual([2, 1, 0]);
      expect(
        apiRef.current.getRowGroupChildren({ groupId, applyFiltering: true })
      ).toEqual([1, 2]);
      expect(
    ...
    it("should return the rows in group of depth 0 from tree of depth 2", () => {
      ...
      ).toEqual(["auto-generated-row-category1/Cat A-category2/Cat 2", 2, 1]);
    });
    ...
    it("should return the rows in group of depth 1 from tree of depth 2", () => {
      ...
      expect(apiRef.current.getRowGroupChildren({ groupId })).toEqual([1, 2]);
      expect(
        apiRef.current.getRowGroupChildren({ groupId, applySorting: true })
      ).toEqual([2, 1]);
      expect(
  ...
  describe("accessibility", () => {
    ...
    //   expect(getRow(0).getAttribute("aria-level")).toEqual("1"); // Cat A
    //   expect(getRow(1).getAttribute("aria-level")).toEqual("2"); // Cat 1
    //   expect(getRow(1).getAttribute("aria-posinset")).toEqual("1");
    //   expect(getRow(1).getAttribute("aria-setsize")).toEqual("2"); // Cat A has Cat 1 & Cat 2
    //   expect(getRow(2).getAttribute("aria-level")).toEqual("3"); // Cat 1 row
    //   expect(getRow(3).getAttribute("aria-posinset")).toEqual("2"); // Cat 2
    //   expect(getRow(4).getAttribute("aria-posinset")).toEqual("1"); // Cat 2 row
    //   expect(getRow(4).getAttribute("aria-setsize")).toEqual("2"); // Cat 2 has 2 rows
    // });
  });
  ...
  it("should properly update the rows when they change", async () => {
    ...
    await waitFor(() =>
      expect(getCell(1, 3).textContent).toEqual("username 2")
    );
  });
  ...
  it("should not collapse expanded groups after `updateRows`", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("button", { name: "hide children" })
      ).toBeVisible();
    });
    await waitFor(() =>
      expect(getCell(1, 3).textContent).toEqual("username 2")
  ...
  //   expect(getColumnValues(3)).toEqual(["", "username1", "username2"]);
