...
> = (state, props, apiRef) => {
  ...
  return {
    ...
    aggregation: {
      model:
        props.aggregationModel ?? props.initialState?.aggregation?.model ?? {},
    },
  };
};
...
export const useGridAggregation = (
  ...
  const setAggregationModel = React.useCallback<
    GridAggregationApi["setAggregationModel"]
  >(
    (model) => {
  ...
  const checkAggregationRulesDiff = React.useCallback(() => {
    ...
    if (
      ...
      apiRef.current.caches.aggregation.rulesOnLastColumnHydration =
        aggregationRules;
