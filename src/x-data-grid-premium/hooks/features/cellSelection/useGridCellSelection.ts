...
> = (state, props) => ({
  ...state,
  cellSelection: {
    ...(props.cellSelectionModel ?? props.initialState?.cellSelection),
  },
});
...
export const useGridCellSelection = (
  ...
  const isCellSelected = React.useCallback<
    GridCellSelectionApi["isCellSelected"]
  >(
    (id, field) => {
      if (!props.cellSelection) {
        return false;
  ...
  const setCellSelectionModel = React.useCallback<
    GridCellSelectionApi["setCellSelectionModel"]
  >(
    (newModel) => {
      if (!props.cellSelection) {
        return;
  ...
  const selectCellRange = React.useCallback<
    GridCellSelectionApi["selectCellRange"]
  >(
  ...
  const startAutoScroll = React.useCallback(() => {
    ...
    function autoScroll() {
      ...
      const { width, height: viewportOuterHeight } =
