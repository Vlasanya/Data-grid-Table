 ...
 */
const groupingFieldIndexComparator: GridComparatorFn = (
  v1,
  v2,
  cellParams1,
  cellParams2
  ...
  const groupingField1 =
    (cellParams1.rowNode as GridGroupNode).groupingField ?? null;
  const groupingField2 =
    (cellParams2.rowNode as GridGroupNode).groupingField ?? null;
...
const getLeafProperties = (leafColDef: GridColDef): Partial<GridColDef> => ({
  ...
  valueOptions: isSingleSelectColDef(leafColDef)
  ...
  sortComparator: (v1, v2, cellParams1, cellParams2) => {
    ...
    if (
      cellParams1.rowNode.type === "leaf" &&
      cellParams2.rowNode.type === "leaf"
    ) {
...
const getGroupingCriteriaProperties = (
  ...
  const properties: Partial<GridColDef> = {
    ...
    valueOptions: isSingleSelectColDef(groupedByColDef)
      ? groupedByColDef.valueOptions
      : undefined,
    sortComparator: (v1, v2, cellParams1, cellParams2) => {
      ...
      if (
        ...
        const colDef = cellParams1.api.getColumn(
  ...
  const commonProperties: Partial<GridColDef> = {
    ...
    width: Math.max(
      ...rowGroupingModel.map(
        (field) =>
          (columnsLookup[field].width ?? GRID_STRING_COL_DEF.width!) + 40
      ),
      leafColDef?.width ?? 0
    ),
    renderCell: (params) => {
