...
function findSkeletonRowsSection({
  ...
  while (!isSkeletonSectionFound && firstRowIndex < lastRowIndex) {
    const isStartingWithASkeletonRow =
      apiRef.current.getRowNode(visibleRowsSection[startIndex].id)?.type ===
      "skeletonRow";
    const isEndingWithASkeletonRow =
      apiRef.current.getRowNode(visibleRowsSection[endIndex].id)?.type ===
      "skeletonRow";
...
export const useGridLazyLoader = (
  ...
  const handleGridSortModelChange = React.useCallback<
    GridEventListener<"sortModelChange">
  >(
    (newSortModel) => {
      if (isDisabled) {
        return;
  ...
  useGridApiEventHandler(
    privateApiRef,
    "sortModelChange",
    handleGridSortModelChange
  );
  useGridApiEventHandler(
    privateApiRef,
    "filterModelChange",
    handleGridFilterModelChange
  );
