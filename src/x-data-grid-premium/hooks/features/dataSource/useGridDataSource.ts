...
import { GridDataSourceCache } from "@mui/x-data-grid/internals";
...
export const useGridDataSource = (
  ...
  const groupsToAutoFetch = useGridSelector(
    apiRef,
    gridRowGroupsToFetchSelector
  );
  ...
  const fetchRows = React.useCallback(
    async (parentId?: GridRowId) => {
      ...
      const cachedData =
        apiRef.current.unstable_dataSource.cache.get(fetchParams);
  ...
  const fetchRowChildren = React.useCallback<
    GridDataSourcePrivateApi["fetchRowChildren"]
  >(
    async (id) => {
      if (!props.treeData) {
      ...
      try {
        ...
        apiRef.current.unstable_dataSource.setChildrenFetchError(
          id,
          childrenFetchError
        );
  ...
  const setChildrenLoading = React.useCallback<
    GridDataSourceApiBase["setChildrenLoading"]
  >(
    (parentId, isLoading) => {
  ...
  const setChildrenFetchError = React.useCallback<
    GridDataSourceApiBase["setChildrenFetchError"]
  >(
    (parentId, error) => {
      apiRef.current.setState((state) => {
