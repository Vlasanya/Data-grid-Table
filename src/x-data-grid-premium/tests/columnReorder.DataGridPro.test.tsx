...
describe("<DataGridPro /> - Columns reorder", () => {
  ...
  it("resizing after columns reorder should respect the new columns order", async () => {
    ...
    const { rerender } = render(<TestCase width={300} />);

    expect(getColumnHeadersTextContent()).toEqual(["id", "brand"]);
    act(() => apiRef.current.setColumnIndex("id", 1));
    rerender(<TestCase width={200} />);
    await raf();
    expect(getColumnHeadersTextContent()).toEqual(["brand", "id"]);
  });
  ...
  it("should not reset the column order when a prop change", () => {
    ...
    expect(getColumnHeadersTextContent()).toEqual(["brand", "desc", "type"]);
    act(() => apiRef.current.setColumnIndex("brand", 2));
    expect(getColumnHeadersTextContent()).toEqual(["desc", "type", "brand"]);
    rerender(<Test />);
    expect(getColumnHeadersTextContent()).toEqual(["desc", "type", "brand"]);
  });
  ...
  it("should keep the order of the columns when dragStart is fired and disableColumnReorder=true", () => {
    ...
    expect(columnHeaderDraggableContainer).not.toHaveClass(
  ...
  it("should call onColumnOrderChange after the column has been reordered", () => {
    ...
    const lastCallArgs = onColumnOrderChange.mock.calls[0];
    expect(lastCallArgs[2].api.state.columns.orderedFields).toEqual([
      "currencyPair",
      "price1M",
      "id",
    ]);
  ...
  describe("column - disableReorder", () => {
    it("should not allow to start dragging a column with disableReorder=true", () => {
      ...
      expect(dragCol).toHaveAttribute("draggable", "false");
      expect(dragCol).not.toHaveClass(gridClasses["columnHeader--dragging"]);
  ...
  describe("reorder with column grouping", () => {
    ...
    describe("column - hidden", () => {
      ...
      it("should consider moving the column between hidden columns if it respect group constraint and visible behavior", () => {
        ...
        rerender(<Test columnVisibilityModel={{}} />);
        expect(getColumnHeadersTextContent()).toEqual([
          "col23",
          "",
          "col2",
