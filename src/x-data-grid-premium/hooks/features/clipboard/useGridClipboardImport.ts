...
import { GRID_REORDER_COL_DEF } from "../../../hooks/features/rowReorder/gridRowReorderColDef";
...
class CellValueUpdater {
  ...
  updateCell({
    ...
    if (colDef.pastedValueParser) {
      parsedValue = colDef.pastedValueParser(
        pastedCellValue,
        row,
        colDef,
        apiRef
...
function defaultPasteResolver({
  ...
  const isSingleValuePasted =
    pastedData.length === 1 && pastedData[0].length === 1;
  ...
  if (cellSelectionModel && selectedCellsArray.length > 1) {
    ...
    selectedCellsArray.forEach(({ id: rowId, field }) => {
      ...
      if (hasRowData) {
        const cellValue = isSingleValuePasted
          ? rowDataArr[0]
          : rowDataArr[colIndex];
  ...
  const visibleColumnFields = gridVisibleColumnFieldsSelector(apiRef).filter(
    (field) => {
  ...
  const selectedRowIndex =
    apiRef.current.getRowIndexRelativeToVisibleRows(selectedRowId);
  const visibleRowIds =
...
export const useGridClipboardImport = (
  ...
  const checkIfCanStartEditing = React.useCallback<
    GridPipeProcessor<"canStartEditing">
  >(
    (initialValue, { event }) => {
  ...
  useGridApiOptionHandler(
    apiRef,
    "clipboardPasteStart",
    props.onClipboardPasteStart
  );
  useGridApiOptionHandler(
    apiRef,
    "clipboardPasteEnd",
    props.onClipboardPasteEnd
  );

  useGridRegisterPipeProcessor(
    apiRef,
    "canStartEditing",
    checkIfCanStartEditing
  );
};
