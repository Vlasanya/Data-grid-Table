...
export const GRID_ROW_GROUPING_SINGLE_GROUPING_FIELD =
...
export const getRowGroupingCriteriaFromGroupingField = (
  groupingColDefField: string
) => {
  const match = groupingColDefField.match(
    /^__row_group_by_columns_group_(.*)__$/
  );
...
const shouldApplyFilterItemOnGroup = (
  ...
  const groupingCriteriaField =
    getRowGroupingCriteriaFromGroupingField(columnField);
    ...
    if (isRowMatchingFilters && node.type !== "footer") {
      const shouldApplyItem =
        node.type === "group" && node.isAutoGenerated
          ? (columnField: string) =>
              shouldApplyFilterItemOnGroup(columnField, node)
          : undefined;
...
export const setStrategyAvailability = (
  ...
  if (disableRowGrouping) {
    ...
    isAvailable = () => {
      const rowGroupingSanitizedModel =
        gridRowGroupingSanitizedModelSelector(privateApiRef);
...
export const getCellGroupingCriteria = ({
  ...
  if (groupingRule.groupingValueGetter) {
    key = groupingRule.groupingValueGetter(
