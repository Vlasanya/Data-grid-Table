...
export const useGridColumnPinning = (
  ...
  >(
    (initialValue, params) => {
      ...
      if (
        !params.colIndex ||
        (visiblePinnedColumns.left.length === 0 &&
          visiblePinnedColumns.right.length === 0)
      ) {
        return initialValue;
      }
      ...
      const leftPinnedColumnsWidth =
        columnPositions[visiblePinnedColumns.left.length];
      const rightPinnedColumnsWidth =
        columnsTotalWidth -
        columnPositions[
          columnPositions.length - visiblePinnedColumns.right.length
        ];
  ...
  >(
    (initialValue, { targetIndex }) => {
      ...
      if (
        visiblePinnedColumns.left.length > 0 &&
        targetIndex < visiblePinnedColumns.left.length
      ) {
        return false;
      }
  ...
  >(
    (
      params,
      context: GridRestoreStatePreProcessingContext<GridInitialStatePro>
    ) => {
