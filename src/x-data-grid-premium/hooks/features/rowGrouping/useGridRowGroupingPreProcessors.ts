...
export const useGridRowGroupingPreProcessors = (
      ...
      switch (props.rowGroupingColumnMode) {
        ...
        case "multiple": {
          return rowGroupingModel.map((groupingCriteria) =>
            createGroupingColDefForOneGroupingCriteria({
              groupingCriteria,
              colDefOverride: getColDefOverrides(groupingColDefProp, [
                groupingCriteria,
              ]),
  ...
  >(
    (columnsState) => {
      ...
      const startIndex =
        newColumnFields[0] === GRID_CHECKBOX_SELECTION_FIELD ? 1 : 0;
      newColumnFields = [
    ...
    (params) => {
      ...
      const getRowTreeBuilderNode = (rowId: GridRowId) => {
        ...
        const parentPath = groupingRules
          ...
          .filter(
            (cell) => cell.key != null
          ) as RowTreeBuilderGroupingCriterion[];
